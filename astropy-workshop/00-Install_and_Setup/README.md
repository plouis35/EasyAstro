...
## 1. Install Miniconda (if needed) or Miniforge (recommanded)

*Miniconda and Miniforge are free minimal installers for conda. There are small, bootstrap
...
In a terminal window, check if Miniconda/Miniforge are already installed.

    % conda info

If Miniconda or Miniforge are not already installed, follow these instructions for your
...
And finally, on any platform, to install and activate the astropy-workshop environment, type:
    ...
    % conda env create --file environment.yml (or update if you want to use the existing 'base' env 
