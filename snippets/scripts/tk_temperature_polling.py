...
# Driven by polling the sensor for a new value every 2 seconds
 

import sys
from time import gmtime, strftime  # for utc
from time import sleep       # for delays 
from Phidget22.Devices.TemperatureSensor import *   # for IR sensor
from Phidget22.PhidgetException import *       # for Phidgets management 
from Phidget22.Phidget import *    # for Phidgets management
...
# Handle the event when the device is attached
...
# Handle the event where the device is disconnected
...
# If an error is found make  it known to the console

def ErrorEvent(self, eCode, description):
  print(("Error %i : %s" % (eCode, description)))


# Measure the time and temperature in a tk mainloop and reschedule it again
...
def measure_temperature():
  ...
  if polling_flag:
    ...
    temperature_celsius = ch.getTemperature() 
    time_utc = utcnow()
  ...
  root.after(2000, measure_temperature)
...
#
# GUI mainloop has a delayed measurement every 2 seconds
# The measurement is rescheduled after every call
#
