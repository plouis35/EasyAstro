...
if len(sys.argv) == 1:
  print(" ")
  print("Usage: fits_autocorrelate.py  outprefix infile1.fits infile2.fits ...   ")
  print(" ")
  sys.exit("Create a stack of autocorrelated images from stack of  fits images\n ")
elif len(sys.argv) >=5:
...
# Set a flag to shift the autocorrelation to the center of the image
# Otherwise the autocorrelation is based at the corner
...
for infile in infiles:
  ...
  ftimage = np.fft.fft2(inimage)
  prodimage = ftimage * ftimage.conj()
  
  if shiftflag:  
    acimage = np.fft.fftshift(np.fft.ifft2(prodimage))
  else:
    acimage = np.fft.ifft2(prodimage)
...
for i in range(nout):
  ...
  outhdr['history'] = 'Autocorrelation slice %d of %d images by fits_autocorrelate' %(i+1,nout)
