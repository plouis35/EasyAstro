...
# Create correlation stack from a stack of uniformly temporally cadenced images
...
if len(sys.argv) == 1:
  print(" ")
  print("Usage: fits_correlate.py  outprefix infile1.fits infile2.fits ...   ")
  print(" ")
  sys.exit("Create a correlation stack from a temporal stack of  fits images\n ")
elif len(sys.argv) >=5:
...
for infile in infiles:
  ...
  if nin == 0:
    ...
    template = inimage
    fttemplate = np.fft.fft2(template)
  ...
  prodimage = fttemplate * ftimage.conj()
  ccimage = np.fft.fftshift(np.fft.ifft2(prodimage))
  outimage = ccimage.real
...
for i in range(nout):
  ...
  outhdr['history'] = 'Cross correlation slice %d of %d images by fits_correlate' %(i+1,nout)
